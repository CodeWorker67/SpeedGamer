"""Бизнес-логика лимита трафика на белой ноде Антиглушилка."""
...
async def apply_wl_subscription_bonus(sql, x3, billing_uid: int, duration_days: int) -> None:
    """PRO-подписка: +WL-трафик по сроку и restore squads, если лимит снова в норме."""
...
def _strip_pro_suffix(username: str) -> str:
...
def billing_uid_from_panel_username(username: str) -> Optional[int]:
    """Telegram/site user_id из username панели; None для white/gift/неизвестных."""
    if not username:
        return None
    if "_white" in username or username.startswith("gift_"):
        return None
    base = _strip_pro_suffix(username)
    try:
        return int(base)
    except ValueError:
        if base.startswith("n") and len(base) > 1:
            try:
                return int(base[1:])
            except ValueError:
                return None
...
def pro_panel_usernames_for_billing_uid(billing_uid: int) -> tuple[str, ...]:
    """Все PRO-usernames в панели для одного billing user (3 / 5 / 10 устройств)."""
    if billing_uid > 0:
        uid = billing_uid
        return (str(uid), f"{uid}_3", f"{uid}_10")
    return (
        panel_username_for_site_user(billing_uid, white=False, device_slots=5),
        panel_username_for_site_user(billing_uid, white=False, device_slots=3),
        panel_username_for_site_user(billing_uid, white=False, device_slots=10),
    )
...
async def fetch_all_pro_panel_users(x3, billing_uid: int) -> list[dict]:
    """Все существующие PRO-аккаунты пользователя в панели."""
    result: list[dict] = []
    for username in pro_panel_usernames_for_billing_uid(billing_uid):
...
def wl_day_gb_for_panel_users(
    ...
    """Суммарный расход за WL-день по всем PRO-подпискам пользователя."""
    total = 0.0
...
async def get_wl_used_gb_for_user(
    ...
    *,
    ...
    """trafic_wl из БД + суммарный расход за WL-день по всем PRO-подпискам."""
...
async def reassign_all_pro_to_active(x3, billing_uid: int) -> int:
    """Вернуть все PRO-подписки на active squad; возвращает число обновлённых."""
    updated = 0
    for panel_user in await fetch_all_pro_panel_users(x3, billing_uid):
...
async def reassign_all_pro_to_limited(x3, billing_uid: int) -> int:
    """Перевести все PRO-подписки на limited squad; возвращает число обновлённых."""
    updated = 0
    for panel_user in await fetch_all_pro_panel_users(x3, billing_uid):
...
async def restore_pro_squads_if_under_limit(
    ...
    if used_gb > limit_gb:
        return 0
    return await reassign_all_pro_to_active(x3, billing_uid)
